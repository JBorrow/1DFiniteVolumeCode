...
void setup_state_single_cell(
    struct cell *current_cell, struct cell *right_neighbour,
...
void convert_conserved_to_primitive_single_cell(struct cell *cell)
{
    ...
    const float kinetic_energy = 0.5f * cell->density * cell->density * cell->velocity;
...
void setup_all_cells(struct cell *cells)
{
    for (int i = 0; i < NUMBER_OF_CELLS; i++)
    {
        ...
        const int left_neighbouring_cell = (i + NUMBER_OF_CELLS - 1) % NUMBER_OF_CELLS;
        const int state = i - NUMBER_OF_CELLS / 2;

        setup_state_single_cell(&cells[i], &cells[right_neighbouring_cell], &cells[left_neighbouring_cell], state);
    }
...
void step(struct cell *cells)
{
    ...
    for (int i = 0; i < NUMBER_OF_CELLS; i++)
    {
        ...
        riemann_solver_solve(&left_state[0], &right_state[0], &output_state[0], &n_unit[0]);
        ...
        const float flux_energy = ((pressure_S * gamma_ratio) + kinetic_energy) * velocity_S;
...
void print_state_of_cells(struct cell *cells)
{
    ...
    for (int i = 0; i < NUMBER_OF_CELLS; i++)
    {
        printf("%e, %e, %e, %e, %e, %e, %e\n",
               cells[i].volume, cells[i].mass, cells[i].momentum, cells[i].energy,
               cells[i].density, cells[i].velocity, cells[i].pressure);
    }
}
